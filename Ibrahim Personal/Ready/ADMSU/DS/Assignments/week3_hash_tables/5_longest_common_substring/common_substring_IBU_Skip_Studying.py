...
'''
To Solve this problem Apply Binary search for half the length of shorter substring 
and calculate the hash values for len(shorter_substring)/2 for both the strings.
If the string of len(shorter_substring)/2 is found in longer substring increase the 
length to len(shorter_substring)/2 + 1 or else decrease the length to len(shorter_substring)/2 - 1
and repeat untill the max matching substring is found


Can be solved in O|S||T| using edit distance dynamic programming 
Running time is linear as Rabin-Karp's algo is repeated in the worst case of len(shorter_substring)/2
if there is no common substrings but still it is good enough as it is fixed (linear) 
and not exponential or quadratic
'''
