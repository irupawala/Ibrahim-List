...
class Eventfire():
    ...
    # Most-Optimal
    # The reason this is the most optimal is because in the sub-optimal approach one acquires and releases lock many times which is very memory heavy for the system
    # In this approach the logic is that only isEventFired is protected in a lock
    # and Once the queue is popped the queue is faster hence queue will be popped off without interruption from any threads 
    def fire(self):
        self.lock.acquire()
        self.isEventFired = True
        self.lock.relase()
