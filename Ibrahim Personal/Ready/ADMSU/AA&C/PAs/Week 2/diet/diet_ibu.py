...
#        usedIndex = [i for i in range(l) if ((x / 2 ** i) % 2) // 1 == 1] # Very fine and advanced method of generating powerset see power_set_bessie.py to understand
        ...
        if len(usedIndex) != mm: # Only consider different combinations of power_set having length = mm
            continue
        lastEquation = False
...
if anst == 1: # if anst = 1 this means that equation a1+a2+.. <= 10^9 is involved and hence one variable is <= 10^9
    print("Infinity")
...
Time Complexity = O(2^(n+m)(m^3 + mn))

########################################################################

Notes:
    
* Uses the concept that optimal solution of a polytope is always on one of the vertex
* with n inequalities there are total l = n + m + 1 inequalities, m for each variable, inequality variable > 0 and 1 for the case of infinite solution where a1+a2+... > 0
* Use powerset to create all subsets of the set of inequalities and select only those subsets which has length equal to mm
* Using these subsets solve the equation and get one of the solution of the equation
* Check if this set of solution satisfies all the inequalities and then calculate obejctive function value. Iteratively calculate objective funtion value for all subsets with length = m
* Select the solution set which gives the largest objective function value as the final result.
* for the case when the ans is "No Solution", the result of the gaussian eliminiation function is such that it provides varaibles values which does not satisfy all the inequalities hence ans is set to -1
* for the case when the ans is "Infinity" lastEquation is involved in the calcutaion of objective function which means that sum of the variables > 10^9 and hence it has infinite solutions
* Refer the code "diet_ibu_study" to print all variables at all steps
