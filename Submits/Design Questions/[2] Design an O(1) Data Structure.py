'''
Question - Implement a Data Structure, so that 
            add, 
            remove, 
            lookup, 
            clear 
            operations would have O(1) time complexity, iteration would have O(size) complexity.
'''
...
class RandomizedSet:
    ...
    def __init__(self):
        self.randomDict = dict()

    def insert(self, val: int) -> bool:
        if val not in self.randomDict:
            self.randomDict[val] = None
            return True
        else:
            return False
    ...
    def getRandom(self) -> int:
        n = random.randint(0, len(self.randomDict)-1)
        return list(self.randomDict)[n]
